...
function updatePlayerSize(w) {
  if (w === 35) return w * 0.5;
  if (w === 30) return w * 0.45;
  if (w === 25) return w * 0.4;
  if (w === 20) return w * 0.35;
  return w * 0.5;
}

function sketch(p) {
  p.setup = function () {
    p.createCanvas(750, 750); // Установлен размер канваса 750x750
    ...
    setInterval(() => {
      timer--;
      if (timer < 0) timer = interval / 1000;
    ...
    if (kaomojis[kaomojiIndex] === "w(°ｏ°)w") {
      ...
      if (surpriseActive && (moveX !== 0 || moveY !== 0)) {
        ...
        if (surpriseDuration <= 0) {
          ...
          player.x = grid[randomCell].i * w + w / 2;
