let words = [];
let wordInput, meaningInput, description, generateBtn, submitBtn, wordWall;
...
function setup() {
  ...
  meaningInput = select('#meaning-input');
...
function generateKaomoji() {
  ...
  wordInput.value(kaomoji);
  meaningInput.value(emotion);
  generateEmotionText(emotion).then(text => {
    console.log("Generated description:", text);
    description.html(text);
  }).catch(error => {
    console.error("Description error:", error);
    description.html(`Ошибка: ${error.message}`);
  });
}
...
function createWordBlock(word, meaning, description) {
  ...
  wordBlock.mouseOver(() => {
    console.log("Hover on kaomoji:", word, "Emotion:", meaning, "Description:", description);
